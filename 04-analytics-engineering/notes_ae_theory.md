...
- [Analytics Engineering](#analytics-engineering)
  ...
  - [dbt Models](#dbt-models)
    ...
    - [Materializations in dbt](#materializations-in-dbt)
    - [Data Sources in dbt: Sources, references and seeds](#data-sources-in-dbt-sources-references-and-seeds)
      - [sources](#sources)
      - [seeds](#seeds)
      - [refs](#refs)
    - [Macros](#macros)
  ...
  - [Deployment of dbt Models](#deployment-of-dbt-models)
...
We will focus on the last two areas: data modeling and presentation.
...
Other approaches such as reducing redundant data (prioritized by other approaches) are secondary. We are not going to focus heavily on making sure that data is not redundant; instead, we prioritize user understandability of this data and query performance.
...
- **Fact tables**: The central table in the schema. Contains quantitative data, metrics, or measurements that represent business processes (e.g., sales, revenue, sessions, or inventory counts). Includes foreign keys that link to the dimension tables.
...
- **dbt Cloud**: SaaS application to develop and manage dbt projects.
  ...
  - Use of semantic layer

#### How are we going to use dbt?

There are 3 different ways in which we can think about using dbt:
...
- **Version C** uses BigQuery. In this approach, you'll perform development using your own IDE, such as VS Code, and install dbt Core locally connecting to the BigQuery database. You will be running dbt models through the CLI
...
![ae6](images/ae6.jpg){: width="600"}

## dbt Models

_[Video source](https://www.youtube.com/watch?v=ueVy2N54lyc)_

DBT sits on top of our platform, either BigQuery or Postgres. We already loaded the raw data as the trips data. Now we're going to work on development, testing, and documentation. We'll start with development and by the end of these lessons, we'll deploy this into production to use those transformations in our BI tools.
...
### Data Sources in dbt: Sources, references and seeds
